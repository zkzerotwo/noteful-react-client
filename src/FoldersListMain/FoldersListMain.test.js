...
import FoldersListMain from './FoldersListMain'

Enzyme.configure({ adapter: new Adapter() });
describe(`FoldersListMain component`, () => {
    const props = {
        notes: [
            {
    ...
    it('renders a .FoldersListMain by default', () => {
        const wrapper = shallow(<FoldersListMain />)
        expect(toJson(wrapper)).toMatchSnapshot()
    })

    it('renders a Note in ul for each notes in array', () => {
        const ul = shallow(<FoldersListMain {...props} />)
            .find('ul')
